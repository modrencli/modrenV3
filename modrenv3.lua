...
if msgs(msg.sender.user_id) == msgs(Themodrenv3) then
...
elseif msgs(msg.sender.user_id) == msgs(Themodrenv3) then
...
local post_count = msgs(Redis:get(Themodrenv3.."modrenv3:Spam:Cont"..msg.sender.user_id..":"..msg_chat_id) or 0)
if post_count >= msgs(Redis:hget(Themodrenv3.."modrenv3:Spam:Group:User"..msg_chat_id,"Num:Spam") or 5) then 
...
Redis:setex(Themodrenv3.."modrenv3:Spam:Cont"..msg.sender.user_id..":"..msg_chat_id, msgs(5), post_count+1) 
...
if msgs(Redis:get(Themodrenv3.."modrenv3:SADD:NUM"..msg.chat_id..msg.sender.user_id)) >= 3 then
Redis:del(Themodrenv3.."modrenv3:SADD:NUM"..msg.chat_id..msg.sender.user_id)
Redis:del(Themodrenv3.."modrenv3:Game:Estimate"..msg.chat_id..msg.sender.user_id)
...
if msgs(Themodrenv3) ~= msgs(FilesJson.BotId) then
return LuaTele.sendText(msg_chat_id,msg_id,'🛡┇عذرا هاذا الملف غير مطابق مع البوت يرجى جلب النسخه الحقيقيه')
end -- end botid
...
local File_Id = Message_Reply.content.document.document.remote.id
local Name_File = Message_Reply.content.document.file_name
if msgs(Name_File:match('(%d+)')) ~= msgs(Themodrenv3) then 
return LuaTele.sendText(msg_chat_id,msg_id,'🛡┇عذرا هاذا الملف غير مطابق مع البوت يرجى جلب النسخه الحقيقيه')
end -- end Namefile
local File = json:decode(https.request('https://api.telegram.org/bot'..Token..'/getfile?file_id='..File_Id)) 
local download_ = download('https://api.telegram.org/file/bot'..Token..'/'..File.result.file_path,''..Name_File) 
...
if msgs(Info_Chats.member_count) < msgs((Redis:get(Themodrenv3..'modrenv3:Num:Add:Bot') or 0)) and not msg.ControllerBot then
...
local CmdStart = '*\n🛡🙋‍♂️┇ اهلا عزيزي'..(Redis:get(Themodrenv3.."modrenv3:Name:Bot") or "بلاك بينك")..
